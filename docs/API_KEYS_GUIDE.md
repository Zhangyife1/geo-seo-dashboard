...
1. **模拟数据模式（默认）**：当未配置任何 API Key 时，CI 环境和爬虫会使用模拟数据兜底，看板可以正常展示，但所有指标均为虚构数据，无法反映真实的品牌可见性情况。
...
**简单来说：不配置 API Key = 看到的全是假数据；配置后 = 看到嗖马 SomaAI 在各 AI 平台的真实曝光情况。**
...
- 如果看到 `Using fallback / mock data`，说明使用了模拟数据兜底。
...
A: 可以。未配置 API Key 的平台会自动跳过 API 采集，回退到浏览器爬虫模式或使用模拟数据。建议至少配置 DeepSeek 和 Kimi 以保证基本的数据覆盖。
