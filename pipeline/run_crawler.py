...
增强版：API 优先 + 浏览器兜底 + 智能补全

流程:
1. 优先通过官方 API 采集（需配置 API Key 环境变量）
2. API 不可用的平台回退到浏览器自动化
3. 浏览器也失败的平台用智能补全数据填充
4. NLP 分析回答内容
5. 结果存入数据库
6. 聚合生成每日指标

使用方法:
    python run_crawler.py              # 运行全部（API优先 + 浏览器兜底）
...
def run_crawl(platform_id: str = None, query_text: str = None, headless: bool = True, mode: str = "auto"):
    ...
    Args:
        ...
        headless: 是否无头模式
        mode: 采集模式 "auto"(API优先+浏览器兜底) / "api"(仅API) / "browser"(仅浏览器)
    """
    init_db()
    ...
    if api_platforms:
        ...
        logger.warning("No API keys configured. Browser mode only (may fail in CI).")
        logger.warning("Set env vars: DEEPSEEK_API_KEY, MOONSHOT_API_KEY, etc.")
    logger.info("=" * 60)
    ...
    for pid in platforms:
                    ...
                    try:
                        ...
                        if result["success"]:
                            nlp_result = analyzer.analyze(
                                result["response_text"], query, pid
                            )
                            ...
                            logger.info("    [API] -> Mentioned: %s, Sentiment: %.2f",
        ...
        if mode in ("auto", "browser") and pid in BROWSER_CRAWLER_MAP:
            ...
            if not pconfig.get("enabled", False) and pid not in BROWSER_CRAWLER_MAP:
                logger.info("  [Browser] Platform %s disabled, skipping browser mode", pid)
                continue
                ...
                for query in queries:
                    ...
                    try:
                        ...
                        if result["success"]:
                            ...
                            logger.info("    [Browser] -> Mentioned: %s, Sentiment: %.2f",
                                       nlp_result["brand_mentioned"],
                                       nlp_result["sentiment_score"])
                        else:
                            CrawlTaskDAO.update_status(
        ...
        if not platform_success:
            logger.warning("  [SKIP] Platform %s: both API and browser failed (or not available)", pid)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, default="auto", choices=["auto", "api", "browser"],
                        help="采集模式: auto(API优先+浏览器兜底) / api(仅API) / browser(仅浏览器)")
