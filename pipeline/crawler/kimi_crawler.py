...
class KimiCrawler(BaseAICrawler):
    def __init__(self, browser_manager):
        ...
        self.platform = "kimi"; self.platform_name = "Kimi"; self.base_url = "https://kimi.moonshot.cn"
    def login(self) -> bool:
        try:
            time.sleep(2)
            return True
        except Exception as e:
            logger.error("[Kimi] %s", e); return True
    def send_query(self, query: str) -> bool:
        try:
            p = self.browser.page
            for sel in ['textarea', '[placeholder*="输入"]', '[placeholder*="Message"]']:
                if p.locator(sel).count() > 0:
                    p.locator(sel).first.fill(query); time.sleep(0.5)
                    p.keyboard.press("Enter"); time.sleep(8); return True
            return False
        except Exception as e:
            logger.error("[Kimi] %s", e); return False
    def get_response_text(self) -> str:
        try:
            p = self.browser.page
            for sel in ['.markdown-body', '[class*="answer"]', '.message-content', '[class*="content"]']:
                els = p.locator(sel).all()
                if els: return els[-1].inner_text()
            return p.inner_text('body')
        except Exception as e:
            logger.error("[Kimi] %s", e); return ""
