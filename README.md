...
看板区分两种数据来源，并在页面横幅与说明中标注：

| 来源 | 含义 |
| --- | --- |
| `real` | 官方 API / 浏览器真实采集的 AI 回答（无论是否提及品牌） |
| `demo` | 预留演示数据：该平台暂无真实采集时用于补齐看板，接入真实 API 后自动替换 |

头部 KPI 提供双口径：`kpis`（全部平台均值）与 `kpis_real`（仅真实采集平台）。当存在真实平台时，页面默认展示真实口径；趋势图 `/api/v1/trend` 只基于真实平台构建，避免演示历史污染。各指标定义见 [docs/DATA_METRICS.md](docs/DATA_METRICS.md)。
